...
void runListeningRefinementTests(Backend *b,QQuickWindow *w){
  ...
  w->resize(800,600);QTest::qWait(200);click("exactVolumeButton");QTest::qWait(250);check(findItem(w->contentItem(),"volumePercent")->isVisible(),"exact volume remains available in narrow player");shot("06-narrow-volume");QTest::keyClick(w,Qt::Key_Escape);
