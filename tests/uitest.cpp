...
void runUiAudit(Backend *b, QQuickWindow *w) {
  ...
  {auto more=findItem(w->contentItem(),"loadMoreButton");check(more&&!more->isVisible(),"empty search hides pagination");}
  ...
  {auto more=findItem(w->contentItem(),"loadMoreButton");check(more&&!more->isVisible(),"failed empty search hides pagination");}
...
void runServerTests(Backend *b,QQuickWindow *w) {
  ...
  if(QCoreApplication::arguments().contains("--jellyfin-test"))click("serverType_jellyfin");
...
void runInteractionTests(Backend *b,QQuickWindow *w) {
  ...
  // The selection bar shortens the list viewport. Aim inside the upper part
  // of row 3 so the drag reaches the list's DropArea above the player bar.
...
void runVisualRefinementTests(Backend *b,QQuickWindow *w){
  ...
  auto shot=[&](const char *name){check(w->grabWindow().save(dir+"/"+name+".png"),name);};
  auto folders=w->findChild<QObject*>("musicFoldersDialog");
  auto click=[&](const char *name){auto item=findItem(w->contentItem(),QString::fromUtf8(name));check(item&&item->isVisible(),name);if(item&&item->isVisible())QTest::mouseClick(w,Qt::LeftButton,Qt::NoModifier,item->mapToScene(QPointF(item->width()/2,item->height()/2)).toPoint());QTest::qWait(80);};
  ...
  for(int i=2;i<=36;++i)QFile::copy(dir+"/music/Track 01.wav",dir+QString("/music/Track %1.wav").arg(i,2,10,QChar('0')));
  b->importMusicFolder(QUrl::fromLocalFile(dir+"/music"));check(until([&]{return !b->importingLocal();}),"import fixture album");
  ...
  QTest::qWait(250);shot("07-home-editor");{auto reset=findItem(w->contentItem(),"homeLayoutReset");check(reset&&!reset->property("needsTooltip").toBool(),"the reset button shows its whole label rather than eliding it");}click("homeLayoutReset");check(b->homeOrder().isEmpty()&&b->hiddenHomeSections().isEmpty(),"Home layout resets");if(editor)QMetaObject::invokeMethod(editor,"close");QTest::qWait(250);
  ...
  w->resize(800,600);QTest::qWait(350);
  auto playerActions=findItem(w->contentItem(),"playerOverflow");
  auto more=playerActions?findItem(playerActions,"appBarOverflow"):nullptr;
...
void runListeningRefinementTests(Backend *b,QQuickWindow *w){
  ...
  b->next();QTest::qWait(140);auto presentation=findItem(w->contentItem(),"trackPresentation");check(b->playbackDirection()==1&&presentation&&presentation->property("direction").toInt()==1,"Next uses forward motion");b->pause();b->seek(0);QTest::qWait(300);b->previous();QTest::qWait(140);check(b->playbackDirection()==-1&&presentation&&presentation->property("direction").toInt()==-1,"Previous uses backward motion");b->pause();b->setMotion(false);b->next();QTest::qWait(50);check(presentation&&presentation->property("offset").toReal()==0&&presentation->property("fade").toReal()==1,"reduced motion changes tracks immediately");b->pause();b->setMotion(true);w->setProperty("side","");
...
void runInteractionRefinementTests(Backend *b,QQuickWindow *w){
  ...
  click("inspectCollectionArtwork");auto viewer=w->findChild<QObject*>("artworkViewer");check(viewer&&viewer->property("visible").toBool(),"artwork viewer opens");
  auto canvas=findItem(w->contentItem(),"artworkCanvas"),inspected=findItem(w->contentItem(),"inspectedArtwork");
  if(canvas&&inspected){
    const auto centered=[&]{return qAbs(inspected->x()-(canvas->property("contentWidth").toReal()-inspected->width())/2)<1 && qAbs(inspected->y()-(canvas->property("contentHeight").toReal()-inspected->height())/2)<1;};
    check(canvas->width()>inspected->width()+20 && centered(),"inspection cover is centered in a wide canvas");shot("artwork-centered");
    for(int step=0;step<3;++step)click("artworkZoomIn");
    check(viewer->property("zoom").toDouble()>1.9&&centered()&&canvas->property("contentWidth").toReal()>canvas->width()+20,"artwork zoom keeps cover centered as it grows beyond the canvas");
    const auto from=canvas->mapToScene(canvas->boundingRect().center()).toPoint();
    QTest::mousePress(w,Qt::LeftButton,Qt::NoModifier,from);
    QTest::mouseMove(w,from-QPoint(90,0),60);
    QTest::mouseRelease(w,Qt::LeftButton,Qt::NoModifier,from-QPoint(90,0));
    QTest::qWait(120);check(canvas->property("contentX").toReal()>20,"dragging zoomed artwork pans it");shot("artwork-zoom");
  }else check(false,"inspection canvas and artwork exist");
