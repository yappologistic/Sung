...
MDialog {
    ...
    // Qt Quick Accessible.announce (Qt 6.8+) reports the keyboard choice
    // while focus stays in the field, where the next key is typed.
    function announceCurrent() {
        if (visible && list.currentIndex>=0 && list.currentIndex<matches.length)
            Accessible.announce(matches[list.currentIndex].title)
    }
    ...
    contentItem: ColumnLayout {
        ...
        MSearchField {
            ...
            onTextChanged: {list.currentIndex=0; dialog.announceCurrent();}
        ...
        ListView {
            ...
            Accessible.name: "Matching quick actions"
            onCurrentIndexChanged: dialog.announceCurrent()
