...
MDialog {
    ...
    ColumnLayout {
        ...
        SungText { objectName: "serverStatus"; text: app.server.error || (app.server.connecting?"Connecting…":app.server.connected?"Connected":"Navidrome / Subsonic"); Layout.fillWidth: true; wrapMode: Text.Wrap; color: app.server.error ? Theme.error : app.server.connected?Theme.primary:Theme.muted }
    ...
    footer: Item {
        ...
        Row {
            ...
            MButton {
                ...
                enabled: !app.server.connecting && address.text.trim().length>0 && username.text.trim().length>0 && password.text.length>0
